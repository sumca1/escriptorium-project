<template>
    <EscrPage class="escr-projects-list">
        <template #page-content>
            <h1>{{ translate('Welcome back') }}, {{ firstName || username }}</h1>
            <div class="escr-card escr-card-table">
                ...
                >
                    <template #modal-content>
                        <h2>{{ translate('Delete Project') }} "{{ projectToDelete.name }}"</h2>
                        <p>
                ...
                >
                    ...
                    >
                        <template #actions="{ item }">
                            <EscrButton
                                v-tooltip.bottom="translate('Delete')"
                                size="small"
                                color="text"
                                :on-click="() => openDeleteModal(item)"
                                :disabled="loading"
                                :aria-label="translate('Delete project')"
                            >
                                <template #button-icon>
                                    <TrashIcon />
                                </template>
                ...
                <EscrLoader
                    v-else
                    :loading="loading"
                    :no-data-message="translate('There are no projects to display.')"
                />
            </div>
        </template>
    </EscrPage>
...
export default {
    ...
    computed: {
        ...
        headers() {
            ...
            return [
                { label: t("Name"), value: "name", sortable: true },
                { label: t("Project Tags"), value: "tags", component: EscrTags },
                { label: t("# of Documents"), value: "documents_count", sortable: true  },
                { label: t("Owner"), value: "owner", sortable: true  },
                {
                    label: t("Last Update"),
                    value: "updated_at",
                    sortable: true,
