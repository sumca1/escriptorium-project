<template>
    <EscrModal class="escr-align-modal">
        ...
        <template #modal-content>
            <h3>{{ translate('Transcription') }}<span
            ...
            />
            <hr>
            <h3>{{ translate('Textual Witness') }}<span
                    :aria-label="translate('required')"
                    class="escr-required"
                >*</span>
            </h3>
            <div class="escr-form-field">
                ...
                <DropdownField
                    ...
                    :label="translate('Textual Witness')"
            ...
            <h3 class="escr-align-settings">
                {{ translate('Settings') }} <EscrButton
                    color="link-secondary"
                    :label="`${showAdvanced ? translate('Hide') : translate('Show')} ${translate('advanced settings')}`"
                    :on-click="showHideAdvanced"
                />
            </h3>
            <TextField
                :disabled="disabled" :help-text="translate('Name for the new transcription layer produced by this alignment.')"
            ...
            <EscrAlert
                ...
                color="danger"
                :message="translate('Reusing an existing layer name will overwrite that layer.')"
            />
            <ArrayField
                :on-change="handleRegionTypesChange"
                :options="regionTypesOptions"
                class="escr-region-types"
                :help-text="translate('Select region types to include in the alignment.')"
                :label="translate('Region Types')"
            />
            <div class="escr-form-field escr-fulldoc-field escr-checkbox-field">
                <label>
                    ...
                    >
                    {{ translate('Use full transcribed document') }}
                </label>
                <span class="escr-help-text">
                    {{ translate('If checked, the aligner will use all transcribed pages of the document to find matches. If unchecked, it will compare each page to the text separately.') }} of the document to find
            ...
            <div class="escr-form-field escr-merge-field escr-checkbox-field">
                <label>
                    ...
                    >
                    {{ translate('Merge aligned text with existing transcription') }}
                </label>
                <span class="escr-help-text">
                    {{ translate('If checked, the aligner will reuse the text of the original transcription when alignment could not be performed; if unchecked, those lines will be blank.') }}
...
export default {
    ...
    computed: {
        ...
        textalWitnessTypeOptions() {
            return [
                {
                    label: this.translate("Select Existing"),
                    value: "select",
                    selected: this.textualWitnessType === "select",
                },
                {
                    label: this.translate("Upload New"),
                    value: "upload",
