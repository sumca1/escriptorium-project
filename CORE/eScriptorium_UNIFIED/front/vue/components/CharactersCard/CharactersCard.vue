    ...
    <div :class="classes">
        ...
        <dl v-if="items.length">
            <div
                ...
                v-tooltip.bottom="item.char ? unicodeCodePoint(item.char) : translate('empty string')"
            >
                <!-- handle space -->
        ...
        <EscrLoader
            v-else
            :loading="loading"
            :no-data-message="translate('There are no characters to display.')"
        />
    </div>
</template>
<script>
...
export default {
    ...
    computed: {
        ...
        sortOptions() {
            return [
                { label: this.translate('by Character'), value: "char" },
                { label: this.translate('by Frequency'), value: "frequency" },
            ].map((sort) => ({
                ...sort,
