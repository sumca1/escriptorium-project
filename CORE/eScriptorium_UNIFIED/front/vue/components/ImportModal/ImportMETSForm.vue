<template>
    <div>
        <h3>{{ translate('Import images, segmentation and transcriptions described by METS') }}</h3>
        <fieldset>
            ...
            <div v-else>
                <TextField
                    :label="translate('Remote METS URI')"
                    :placeholder="translate('Enter METS file URI')"
            ...
            <TextField :label="translate('Transcription Name')" :help-text="translate('The name of the resulting transcription layer.')"
...
export default {
    ...
    computed: {
        ...
        metsOptions() {
            return [
                { value: "url", label: this.translate("Enter URL"), selected: this.metsType === "url" },
                { value: "local", label: this.translate("Upload file"), selected: this.metsType === "local" }
            ];
        },
    },
    methods: {
