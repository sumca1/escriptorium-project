<template>
    <EscrModal :class="`escr-train-modal--${scope}`">
        <template #modal-header>
            <h2>{{ translate('Train') }} {{ scope }} {{ translate('Model') }}</h2>
            <EscrButton
                color="text"
                :on-click="onCancel"
                size="small"
        ...
        <template #modal-content>
            ...
            />
            <DropdownField
                :label="translate('Base Model (optional)')"
                :help-text="translate('You may select an existing model to fine-tune. If left unselected, the model will be trained from scratch.')"
                :disabled="disabled"
            ...
            <label class="escr-form-field model-override-label">
                ...
                >
                {{ translate('Overwrite existing model file') }}
            </label>
        </template>
