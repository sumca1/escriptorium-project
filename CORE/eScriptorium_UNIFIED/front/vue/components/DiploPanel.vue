<template>
    ...
    >
        ...
        >
            <i
                :title="translate('Text Panel')"
                class="panel-icon fas fa-list-ol"
            />
            <i
                id="save-notif"
                ref="saveNotif"
                :title="translate('There is content waiting to be saved (dont leave the page)')"
                class="notice fas fa-save hide"
            />
            <button
                id="sortMode"
                ref="sortMode"
                :title="translate('Toggle sorting mode.')"
            ...
            <button
                ...
                :title="translate('Toggle Virtual Keyboard for this document.')"
                @click="toggleVK"
            >
...
export default {
    ...
    methods: {
        ...
        initAnnotations() {
            ...
            this.anno.on("createAnnotation", async function(annotation) {
                ...
                let offsets = annotation.target.selector.find(
                    (e) => e.type == this.translate("TextPositionSelector")
                );
        ...
        onKeyPress(ev) {
            ...
            if (ev.key == this.translate("ArrowDown") && !ev.shiftKey) {
                ...
                ev.preventDefault();
            } else if (ev.key == this.translate("ArrowUp") && !ev.shiftKey) {
                let sel = window.getSelection();
                let line = sel.anchorNode.nodeType === Node.TEXT_NODE
                    ? sel.anchorNode.parentElement
                    : sel.anchorNode;
