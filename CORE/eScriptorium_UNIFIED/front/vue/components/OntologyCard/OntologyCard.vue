<template>
    <div :class="classes">
        ...
        <EscrLoader
            v-else
            :loading="loading"
            :no-data-message="translate('There is no ontology to display.')"
        />
    </div>
</template>
<script>
...
export default {
    ...
    computed: {
        ...
        categories() {
            return [
                { label: this.translate('Regions'), value: "regions" },
                { label: this.translate('Lines'), value: "lines" },
                { label: this.translate('Text'), value: "text" },
                { label: this.translate('Images'), value: "image" },
            ].map((category) => ({
                ...category,
        ...
        tableHeaders() {
            return [
                { label: this.translate('Type'), value: "label", sortable: true },
                { label: this.translate('# in Document'), value: "frequency", sortable: true },
            ];
        },
    },
    methods: {
