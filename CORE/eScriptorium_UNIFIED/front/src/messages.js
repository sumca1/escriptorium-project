...
export function joinDocumentRoom(pk) {
    if (!msgSocket) {
        console.warn("WebSocket not initialized. Cannot join document room.");
        return;
    }
    
    // Check if WebSocket is already open
    if (msgSocket.readyState === WebSocket.OPEN) {
        msgSocket.send(
            '{"type": "join-room", "object_cls": "document", "object_pk": ' +
                pk +
                " }",
        );
    } else if (msgSocket.readyState === WebSocket.CONNECTING) {
        ...
        const handler = function(e) {
            ...
            msgSocket.removeEventListener("open", handler);
        };
        msgSocket.addEventListener("open", handler);
    } else {
        console.error("WebSocket is in an invalid state:", msgSocket.readyState);
    }
}
