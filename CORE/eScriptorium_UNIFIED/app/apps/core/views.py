...
class AnalyticsDashboard(LoginRequiredMixin, TemplateView):
    """📊 Analytics Dashboard - Overview of All Models"""
    template_name = "core/analytics_overview.html"
    http_method_names = ('get',)
    
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        # Pass all models that the user can access
        context['models'] = OcrModel.objects.filter(
            Q(owner=self.request.user) | Q(public=True) | Q(ocr_model_rights__user=self.request.user)
        ).distinct().select_related('owner', 'script').order_by('-training_accuracy')
        return context
...
# ================================================================
# Proxy views for FastAPI image processing service
# ================================================================
...
def fastapi_auto_process(request):
    """
    Proxy to FastAPI auto-process endpoint.
    
    Processes uploaded image through full pipeline:
    denoise → deskew → enhance → binarize
    
    Returns:
        - If return_json=true: JSON with metadata and base64 image
        - If return_json=false: PNG image file
    """
    ...
    try:
        ...
        # Get parameters
        return_json = request.GET.get('return_json', 'false').lower() == 'true'
        ...
        response = requests.post(
            ...
            params={'return_json': return_json},
            timeout=60
        )
...
@login_required
@csrf_exempt
def fastapi_binarize(request):
    """Proxy to FastAPI binarization endpoint."""
    if request.method != 'POST':
        return JsonResponse({'error': 'Method not allowed'}, status=405)
    ...
    try:
        ...
        method = request.GET.get('method', 'otsu')
        threshold = request.GET.get('threshold')
        return_json = request.GET.get('return_json', 'false').lower() == 'true'
        
        params = {'method': method, 'return_json': return_json}
        if threshold:
            params['threshold'] = threshold
        
        fastapi_url = getattr(settings, 'FASTAPI_URL', 'http://localhost:8001')
        response = requests.post(
            f'{fastapi_url}/api/images/binarize',
            files={'file': (image_file.name, image_file.read(), image_file.content_type)},
            params=params,
            timeout=60
        )
        ...
        logger.error(f"FastAPI binarize error: {e}")
        return JsonResponse({'error': str(e)}, status=500)


@login_required
@csrf_exempt
def fastapi_denoise(request):
    """Proxy to FastAPI denoising endpoint."""
    if request.method != 'POST':
        return JsonResponse({'error': 'Method not allowed'}, status=405)
    ...
    try:
        ...
        h = request.GET.get('h', '10')
        return_json = request.GET.get('return_json', 'false').lower() == 'true'
        
        fastapi_url = getattr(settings, 'FASTAPI_URL', 'http://localhost:8001')
        response = requests.post(
            f'{fastapi_url}/api/images/denoise',
            files={'file': (image_file.name, image_file.read(), image_file.content_type)},
            params={'h': h, 'return_json': return_json},
            timeout=60
        )
        ...
        logger.error(f"FastAPI denoise error: {e}")
        return JsonResponse({'error': str(e)}, status=500)


@login_required
@csrf_exempt
def fastapi_deskew(request):
    """Proxy to FastAPI deskewing endpoint."""
    if request.method != 'POST':
        return JsonResponse({'error': 'Method not allowed'}, status=405)
    ...
    try:
        ...
        fastapi_url = getattr(settings, 'FASTAPI_URL', 'http://localhost:8001')
        response = requests.post(
            f'{fastapi_url}/api/images/deskew',
            files={'file': (image_file.name, image_file.read(), image_file.content_type)},
            params={'return_json': return_json},
            timeout=60
        )
        ...
        logger.error(f"FastAPI deskew error: {e}")
        return JsonResponse({'error': str(e)}, status=500)


@login_required
@csrf_exempt
def fastapi_enhance(request):
    """Proxy to FastAPI contrast enhancement endpoint."""
    if request.method != 'POST':
        return JsonResponse({'error': 'Method not allowed'}, status=405)
    ...
    try:
        ...
        clip_limit = request.GET.get('clip_limit', '2.0')
        tile_size = request.GET.get('tile_size', '8')
        return_json = request.GET.get('return_json', 'false').lower() == 'true'
        
        fastapi_url = getattr(settings, 'FASTAPI_URL', 'http://localhost:8001')
        response = requests.post(
            f'{fastapi_url}/api/images/enhance',
            files={'file': (image_file.name, image_file.read(), image_file.content_type)},
            params={
        ...
        logger.error(f"FastAPI enhance error: {e}")
        return JsonResponse({'error': str(e)}, status=500)


@login_required
def fastapi_health(request):
    """Check FastAPI service health status."""
    try:
        fastapi_url = getattr(settings, 'FASTAPI_URL', 'http://localhost:8001')
...
class ModelCheckerView(LoginRequiredMixin, TemplateView):
    """
    🔍 Model Checker - Web UI for eScriptorium Model Analysis
    
    Features:
    - Upload and analyze OCR models
    - Hebrew model detection
    - Charset analysis
    - Quality estimation
    - Model comparison
    """
    template_name = 'core/model_checker.html'
    
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['page_title'] = _('Model Checker')
        return context
    ...
    def post(self, request, *args, **kwargs):
        ...
        app_path = Path(settings.BASE_DIR)
        ...
        if action == 'analyze':
            ...
            try:
                ...
                model = get_object_or_404(OcrModel, pk=model_id)
                
                # Check permissions
                if not model.public and model.owner != request.user:
                    return JsonResponse({'error': 'Permission denied'}, status=403)
                
                # Analyze the model file
                checker = EscriptoriumModelChecker()
                result = checker.check_model(str(model.file.path))
            ...
            if len(model_ids) < 2:
                return JsonResponse({'error': 'Select at least 2 models to compare'}, status=400)
            
            try:
                checker = EscriptoriumModelChecker()
                models = OcrModel.objects.filter(pk__in=model_ids)
                
                # Check permissions
                for model in models:
                    if not model.public and model.owner != request.user:
                        return JsonResponse({'error': f'Permission denied for model: {model.name}'}, status=403)
                
                # Compare models
                model_paths = [str(m.file.path) for m in models]
                comparison = checker.compare_models(model_paths)
            ...
            # Quick scan of user's models
            try:
                # Get user's models directory
                user_models = OcrModel.objects.filter(
                ...
                for model in user_models[:20]:  # Limit to 20 models for performance
                    try:
                        analysis = checker.check_model(str(model.file.path))
                        results.append({
                            'id': model.id,
                            'name': model.name,
                            'analysis': analysis
