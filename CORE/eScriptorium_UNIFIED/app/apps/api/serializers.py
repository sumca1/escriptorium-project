...
class DocumentSerializer(serializers.ModelSerializer):
    main_script = serializers.SlugRelatedField(
        ...
        required=False,  # 🔥 FIX: Make optional like in model (null=True, blank=True)
        allow_null=True
    )
...
class LineTranscriptionSerializer(serializers.ModelSerializer):
    """Serializer for LineTranscription - optimized for embedding in LineSerializer
    
    CRITICAL: The 'line' field MUST be included in both the class definition AND Meta.fields!
    It's dynamically overridden by RuntimeSerializer in LineTranscriptionViewSet.get_serializer_class()
    to filter by document_part. Without it in Meta.fields, DRF raises AssertionError.
    
    See: .github/CRITICAL_FIXES_DOCUMENTATION.md for details.
    """
    transcription = serializers.IntegerField(source='transcription_id', read_only=True)
    line = serializers.PrimaryKeyRelatedField(queryset=Line.objects.all(), required=False, write_only=True)

    class Meta:
        model = LineTranscription
        fields = ('pk', 'line', 'transcription', 'content', 'version_source', 'version_updated_at')
...
class LineSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Line
        fields = ('pk', 'document_part', 'external_id', 'order', 'region', 'baseline', 'mask', 'typology', 'transcriptions')
        extra_kwargs = {
    ...
    def get_transcriptions(self, obj):
        """Return transcriptions for this line, optionally filtered by transcription_id from context."""
        transcription_id = self.context.get('transcription_id')
        qs = obj.transcriptions.all()
        
        if transcription_id:
            qs = qs.filter(transcription_id=transcription_id)
        
        return LineTranscriptionSerializer(qs, many=True).data
...
class OcrModelStatusSerializer(serializers.ModelSerializer):
    job_display = serializers.CharField(source='get_job_display', read_only=True)
    trained_on_documents = serializers.SerializerMethodField()
    ready_for_use = serializers.SerializerMethodField()
    
    class Meta:
        model = OcrModel
        fields = [
            'id', 'name', 'job', 'job_display', 'training', 'file_size',
            'training_accuracy', 'training_total', 'created_at', 'updated_at',
            'trained_on_documents', 'ready_for_use'
        ]
    
    def get_trained_on_documents(self, obj):
        return [
            {
                'id': omd.document.id,
                'name': omd.document.name,
                'trained_on': omd.trained_on
            }
            for omd in obj.ocrmodeldocument_set.select_related('document').all()
        ]
    
    def get_ready_for_use(self, obj):
        return not obj.training and obj.file_size > 0
