...
1. **Django i18n (django.po)** → טקסטים סטטיים מתבניות Django וקוד Python
2. **Typology Translations (typology_translations_he.py)** → שמות טיפולוגיות ממסד נתונים (BlockType, LineType)
3. **Clean BiblIA Middleware (clean_biblia_middleware_v2.py)** → תרגום דינמי של HTML שמגיע מ-Django + RTL
4. **Vue i18n (he.json)** → ממשק Vue.js (Editor, כפתורים, כלים)

**כלל אצבע:**
- אם זה בתבנית Django → `django.po`
- אם זה שם טיפולוגיה (סוג אזור/שורה) → `typology_translations_he.py`
- אם זה קומפוננטת Vue (Editor, כלי ציור) → `front/vue/locales/he.json`
- אם זה כל דבר אחר (HTML דינמי) → `clean_biblia_middleware_v2.py`
