...
class ModelUploadForm(BootstrapFormMixin, forms.ModelForm):
    ...
    def clean_file(self):
        ...
        file_extension = file_field.name.rsplit('.', 1)[-1].lower()
        
        if file_extension == 'traineddata':
            # Tesseract model - minimal validation (just check it's a file)
            self._model_job = 'recognition'
            self.model_metadata = None
            logger.info(f"Tesseract model detected: {file_field.name}")
        elif file_extension == 'mlmodel':
            # Kraken model - full validation
            try:
            ...
            raise forms.ValidationError(_("Unsupported model file format."))
...
class RegionTypesFormMixin(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        ] + [('Undefined', _('(Undefined region type)')), ('Orphan', _('(Orphan lines)'))]
...
class GroupedModelChoiceIterator(ModelChoiceIterator):
    """Iterator for grouped model choices (e.g., grouping by engine type)."""
