...
class Typology(ExportModelOperationsMixin("Typology"), models.Model):
    ...
    def get_localized_name(self, language=None):
        """Return the typology name in the specified language"""
...
class DocumentPartType(Typology):
    class Meta:
        app_label = 'core'
        verbose_name = _('document part type')
        verbose_name_plural = _('document part types')
...
class LineType(Typology):
    class Meta:
        ...
        verbose_name_plural = _('line types')
...
class AnnotationComponent(models.Model):
    ...
    class Meta:
        verbose_name = _('annotation component')
        verbose_name_plural = _('annotation components')
...
class AnnotationTaxonomy(OrderedModel):
    ...
    class Meta(OrderedModel.Meta):
        verbose_name = _('annotation taxonomy')
        verbose_name_plural = _('annotation taxonomies')
        pass
...
class Script(ExportModelOperationsMixin("Script"), models.Model):
    ...
    def get_localized_name(self, language=None):
        """Return the script name in the specified language"""
        from django.utils import translation
...
class Document(ExportModelOperationsMixin("Document"), CascadeUpdate, models.Model):
    ...
    # =========================================================================
    # Language Detection & OCR Model Recommendations (BiblIA Enhancement)
    # =========================================================================

    def detect_language_and_recommend_model(self, sample_size=500):
        """
        Detect the primary language/script of the document and recommend OCR model.
        
        Uses automatic script detection on transcribed text samples to identify
        Hebrew, Arabic, Latin, or other scripts, then recommends the optimal
        OCR engine and model based on the detected script.
        
        Args:
            sample_size: Number of characters to sample from transcriptions
            
        Returns:
            Dictionary with detection results and recommendations:
            {
                'script': 'hebrew' | 'arabic' | 'latin' | ...,
                'direction': 'rtl' | 'ltr',
                'confidence': float (0-1),
                'has_nikud': bool,
                'has_tashkeel': bool,
                'recommended_engine': 'kraken' | 'tesseract',
                'model_hint': str,
                'preprocessing': list,
                'sample_text': str
            }
        ...
        # Collect text samples from existing transcriptions
        sample_text = self._get_text_sample(sample_size)
        ...
        if not sample_text or len(sample_text.strip()) < 10:
            return {
                ...
                'sample_text': '',
                'error': 'No transcribed text available for analysis'
            }
        
        # Detect script
        script = detect_script(sample_text)
        stats = get_script_statistics(sample_text)
        
        # Calculate confidence based on script dominance
        total_chars = sum(stats.values())
        if total_chars > 0:
            script_counts = {
                ScriptType.HEBREW: stats.get('hebrew', 0),
                ScriptType.ARABIC: stats.get('arabic', 0),
                ScriptType.LATIN: stats.get('latin', 0),
                ScriptType.GREEK: stats.get('greek', 0),
                ScriptType.CYRILLIC: stats.get('cyrillic', 0),
            }
            max_count = max(script_counts.values()) if script_counts else 0
            confidence = max_count / total_chars if total_chars > 0 else 0.0
        else:
            confidence = 0.0
        
        # Get recommendations
        recommendations = recommend_ocr_model(sample_text)
        
        # Add additional info
        result = {
            **recommendations,
            'confidence': confidence,
            'sample_text': sample_text[:200],  # First 200 chars
            'statistics': stats,
        }
        
        return result

    def _get_text_sample(self, max_chars=500):
        """
        Extract a sample of transcribed text from the document.
        
        Collects text from existing transcriptions to analyze language/script.
        
        Args:
            max_chars: Maximum number of characters to collect
            
        Returns:
            String of sample text
        """
        from apps.core.models import LineTranscription
        
        sample_parts = []
        total_chars = 0
        
        # Get transcriptions from document parts
        for part in self.parts.all():
            if total_chars >= max_chars:
                break
                
            # Get lines from first available transcription
            transcription = part.transcriptions.first()
            ...
            ).values_list('content', flat=True)[:50]  # Max 50 lines per part
    ...
    def auto_detect_and_set_direction(self):
        """
        Automatically detect and set the read direction based on script.
        
        Analyzes document text to determine if it's RTL (Hebrew/Arabic)
        or LTR (Latin/etc.) and updates the document's read_direction field.
        
        Returns:
            Boolean indicating if direction was changed
        """
        detection = self.detect_language_and_recommend_model()
        
        detected_direction = detection.get('direction', 'ltr')
        
        # Map to document READ_DIRECTION choices
        direction_map = {
            'rtl': self.READ_DIRECTION_RTL,
            'ltr': self.READ_DIRECTION_LTR,
        }
        
        new_direction = direction_map.get(detected_direction, self.READ_DIRECTION_LTR)
        
        if self.read_direction != new_direction:
            self.read_direction = new_direction
            self.save(update_fields=['read_direction'])
            return True
...
class DocumentPart(ExportModelOperationsMixin("DocumentPart"), CascadeUpdate, OrderedModel):
    ...
    class Meta(OrderedModel.Meta):
        verbose_name = _('document part')
        verbose_name_plural = _('document parts')
        pass
    ...
    def transcribe(self, model, transcription, text_direction=None, user=None):
        ...
        if engine == 'tesseract':
            return self.transcribe_tesseract(model, transcription, text_direction, user)
        else:
            return self.transcribe_kraken(model, transcription, text_direction, user)
    ...
    def transcribe_tesseract(self, model, transcription, text_direction=None, user=None):
        ...
        try:
            ...
            logger.error("tesserocr not installed. Cannot use Tesseract OCR.")
            raise ProcessFailureException("Tesseract support not available - tesserocr not installed")
        ...
        # Extract language name from traineddata filename (e.g., "heb.traineddata" -> "heb")
        lang_name = os.path.basename(model.file.path).rsplit('.', 1)[0]
        tessdata_path = os.path.dirname(model.file.path)

        with Image.open(self.image.file.name) as im:
            with PyTessBaseAPI(path=tessdata_path, lang=lang_name, psm=13) as api:
                line_confidences = []
                ...
                for line in lines:
                    ...
                    # Crop line image from full page
                    bll = BaselineLine(id='foo',
                                     baseline=line.baseline,
                                       boundary=line.mask)
                    seg = Segmentation(type='baselines',
                    ...
                    # Simple crop using mask bounding box
                    if line.mask:
                        coords = list(zip(*line.mask))
                    ...
                    if ri:
                        ...
                        for symbol in iterate_level(ri, RIL.SYMBOL):
                            if symbol.IsAtBeginningOf(RIL.WORD) and content != "":
                                content += " "
                            try:
                                char = symbol.GetUTF8Text(RIL.SYMBOL)
                                conf = symbol.Confidence(RIL.SYMBOL) / 100.0  # Convert to 0-1 range
                                bbox_internal = symbol.BoundingBoxInternal(RIL.SYMBOL)
                                ...
                                logger.warning(f"Error processing Tesseract symbol: {e}")
                                continue
                        
                        lt.content = content.strip()
                        lt.graphs = graph
                        
                        if confidences:
                            line_avg_confidence = mean(confidences)
                            lt.avg_confidence = line_avg_confidence
                            line_confidences.append(line_avg_confidence)
                    else:
                        lt.content = ""
        ...
        # overall avg recalculations
        if line_confidences:
            lines_with_confidence = transcription.linetranscription_set.filter(avg_confidence__isnull=False)
            if lines_with_confidence.exists():
                transcription.avg_confidence = lines_with_confidence.aggregate(avg=Avg("avg_confidence")).get("avg")
            else:
                transcription.avg_confidence = avg_line_confidence
            transcription.save()
...
class LineTranscription(
    ...
    class Meta:
        verbose_name = _('line transcription')
        verbose_name_plural = _('line transcriptions')
    ...
    def save(self, *args, **kwargs):
        ...
        try:
            ...
            if not settings.DISABLE_ELASTICSEARCH and self.content:
                ...
                tasks.index_transcription_to_es.delay(self.pk)
        except Exception:
            pass  # Silent fail - אם ES לא זמין
...
class OcrModel(ExportModelOperationsMixin("OcrModel"), Versioned, models.Model):
    ...
    # BiblIA Enhancement: Training Analytics
    training_started_at = models.DateTimeField(null=True, blank=True, help_text="When training started")
    training_completed_at = models.DateTimeField(null=True, blank=True, help_text="When training completed")
    training_duration_seconds = models.IntegerField(null=True, blank=True, help_text="Total training duration in seconds")
    training_total_epochs = models.PositiveSmallIntegerField(default=0, help_text="Total epochs completed")
    training_epoch_history = models.JSONField(null=True, blank=True, help_text="Per-epoch accuracy and loss history")
    ...
    def clone_for_training(self, owner, name=None):
        ...
        model_extension = self.file.name.rsplit('.', 1)[-1]  # Get original extension
        base_name = self.name.split(f".{model_extension}")[0] if f".{model_extension}" in self.name else self.name.split(".")[0]
        name = name or f"{base_name}_v{children_count}"
        ...
        model.file = File(self.file, name=f'{name}.{model_extension}')  # Preserve extension
...
class OcrModelDocument(models.Model):
    ...
    class Meta:
        verbose_name = _('OCR model document')
        verbose_name_plural = _('OCR model documents')
...
class OcrModelRight(models.Model):
    ...
    class Meta:
        verbose_name = _('OCR model right')
        verbose_name_plural = _('OCR model rights')
        constraints = [
...
class TextualWitness(models.Model):
    ...
    class Meta:
        verbose_name = _('textual witness')
        verbose_name_plural = _('textual witnesses')
...
# Added: 2025-10-20
# Import error detection models
# ============================================
from core.models_errors import (  # noqa: E402, F401
    DetectedError,
    ErrorCorrection,
...
def delete_from_elasticsearch(sender, instance, **kwargs):
    ...
    try:
        ...
        if not settings.DISABLE_ELASTICSEARCH:
            from core import tasks
            tasks.delete_transcription_from_es.delay(instance.pk)
    except Exception:
        pass  # Silent fail - אם ES לא זמין


# Import text alignment models (Feature #4)
# ============================================
from core.models_alignment import (  # noqa: E402, F401
    TextAlignment,
    AlignmentPair,
    AlignmentJob,
)
