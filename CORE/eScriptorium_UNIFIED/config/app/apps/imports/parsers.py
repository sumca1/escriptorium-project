...
# Handle pyvips gracefully when libvips is not installed
try:
    import pyvips
    PYVIPS_AVAILABLE = True
except Exception as e:
    # Fallback when libvips not installed (Windows development)
    import sys
    print(f"⚠️  Warning: pyvips not available - {type(e).__name__}: {e}", file=sys.stderr)
    print("   PDF/TIFF import will be limited. See FIX_LIBVIPS_GUIDE.md", file=sys.stderr)
    from . import pyvips_mock as pyvips
...
XML_EXTENSIONS = ["xml", "alto", "abbyy", "hocr"]  # Extended with converter support
...
class XMLParser(ParserDocument):
    ...
    def validate(self):
        if self.schema_location in self.ACCEPTED_SCHEMAS:
            try:
                # Security fix: Added timeout to prevent indefinite hanging
                response = requests.get(self.schema_location, timeout=30)
...
class IIIFManifestParser(ParserDocument):
    ...
    def get_image(url: str, retry_limit: int = 4) -> requests.Response:
        ...
        while current_retry < retry_limit:
            ...
            try:
                # Security fix: Enabled SSL certificate verification
                # verify=False was allowing potential man-in-the-middle attacks
...
def make_parser(document, file_handler, name=None, report=None, zip_allowed=True, pdf_allowed=True, mets_describer=False, mets_base_uri=None):
    ...
    if ext in XML_EXTENSIONS:
        ...
        # Try automatic format conversion using ocr-fileformat
        converter = get_converter()
        if converter.available:
            file_handler.seek(0)
            xml_content = file_handler.read().decode('utf-8')
            detected_format = converter.detect_format(xml_content, file_handler.name)
            
            if detected_format and detected_format not in ['alto', 'page']:
                # Convert ABBYY, hOCR, etc. to PAGE XML
                try:
                    logger.info(
                        f"Auto-converting {file_handler.name} from {detected_format} to PAGE XML"
                    )
                    page_xml_content = converter.convert_to_page_xml(
                        xml_content,
                        detected_format,
                        file_handler.name
                    )
                    
                    # Parse converted PAGE XML
                    root = etree.fromstring(page_xml_content.encode('utf-8'))
                    
                    # Use PagexmlParser with converted content
                    from io import BytesIO
                    converted_handler = BytesIO(page_xml_content.encode('utf-8'))
                    converted_handler.name = file_handler.name.replace(
                        f'.{ext}', '_converted.xml'
                    )
                    
                    if report:
                        report.append(
                            f"✅ Converted {file_handler.name} from {detected_format.upper()} "
                            f"to PAGE XML using ocr-fileformat"
                        )
                    
                    return PagexmlParser(
                        document, converted_handler, report,
                        transcription_name=name, xml_root=root
                    )
                    
                except Exception as e:
                    logger.warning(
                        f"Format conversion failed for {file_handler.name}: {e}. "
                        f"Falling back to standard parsers."
                    )
                    if report:
                        report.append(
                            f"⚠️ Auto-conversion failed: {str(e)}. Using standard parser."
                        )
                    # Fall through to standard parsers
                    file_handler.seek(0)
                    root = etree.parse(file_handler).getroot()
        
        # Standard parsers (fallback or for already-supported formats)
