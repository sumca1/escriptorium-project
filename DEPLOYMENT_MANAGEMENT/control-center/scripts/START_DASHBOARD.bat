...
echo [1/3] Syncing documentation files...
echo.

REM Sync SESSION_LOG.md and CURRENT_STATE.md to dashboard
powershell -ExecutionPolicy Bypass -File "%CONTROL_CENTER_ROOT%\scripts\utilities\sync-docs-to-dashboard.ps1" >nul 2>&1
if %ERRORLEVEL% EQU 0 (
    echo    Documentation synced successfully
) else (
    echo    Warning: Could not sync documentation
)
...
REM Start Terminal Server (port 3000) - in separate window so it stays open
start "Terminal Server (Port 3000)" node "%CONTROL_CENTER_ROOT%\servers\terminal-server.js" 3000
...
echo    Terminal Server: Running in separate window (port 3000)
...
echo ========================================
echo.
echo Terminal Server: http://localhost:3000 (running in separate window)
